...
\begin{stdfig}
	...
	\caption{Zoomed-in version of \Fig{overview} to show the difference in propagation distance $\Delta s$ from the sound source to the two  microphones. When the distance to the source is very long compared to the distance between the microphones,$r_1, r_2 \ll d$, the paths can be approximated as parallel and the difference in distance is 
